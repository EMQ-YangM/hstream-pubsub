...
module Network.MQTT.Trie where
    ...
    insertWith' (x :| xs) (Trie m) = Trie $ M.alter g x m
      where
        g mn = Just $ case xs of
          ...
          (y : ys) -> node (insertWith' (y :| ys) $ maybe empty nodeTree mn) Nothing
...
foldl' f acc (Trie m) = M.foldl' g acc m
  where
    g acc' n =
      flip (foldl' f) (nodeTree n) $! case nodeValue n of
        Nothing -> acc'
    ...
    findTail x [] (Trie m) =
      ...
      where
        matchMultiLevelWildcard =
          M.lookup multiLevelWildcard m
            >>= nodeValue
        matchSingleLevelWildcard =
          M.lookup singleLevelWildcard m >>= \n ->
...
matchTopic tf = matchTopicHead (topicLevels tf)
  where
    ...
    matchTopicHead (x :| xs) t@(Trie m)
      | startsWithDollar x = case xs of
        [] -> matchExact x m
        (y : ys) -> Just True == (matchTopicTail y ys . nodeTree <$> M.lookup x m)
...
matchFilter tf = matchFilter' (filterLevels tf)
  where
    ...
    matchFilter' (x :| y : zs) (Trie m)
      ...
      where
        matchMultiLevelWildcard = M.member multiLevelWildcard m
        matchSingleLevelWildcard = Just True == (matchFilter' (y :| zs) . nodeTree <$> M.lookup singleLevelWildcard m)
        matchExact = Just True == (matchFilter' (y :| zs) . nodeTree <$> M.lookup x m)
